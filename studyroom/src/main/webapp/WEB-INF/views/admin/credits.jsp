    ...
    <script>
        ...
        function searching(){
            ...
            if (userid == "" && reserve1 == "" && reserve2 == "" && cancel1 == "" && cancel2 == "" && sign1 == "" &&
                ...
                if ((cancel1 !="" || cancel1 == 0) && (cancel2 != "" || cancel2 == 0)){
                    if (cancel1 > cancel2){
                        alert("最小值和最大值错误");
                        ok=0;
                    }
                }
                if ((sign1 !="" || sign1==0) && (sign2 != "" || sign2 == 0)){
                    if (sign1 > sign2){
                        alert("最小值和最大值错误");
                        ok=0;
                    }
                ...
                if ((time1 !="" || time1 == 0) && (time2 != "" || time2 == 0)){
                    if (time1 > time2){
                        alert("最小值和最大值错误");
                        ok=0;
                    }
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
                            <a href="/credits?start=0${page.param}" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>

                        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
                            <a href="/credits?start=${page.start-page.count}${page.param}" aria-label="Previous">
                        ...
                        <c:forEach begin="0" end="${page.totalPage-1}" varStatus="status">
                            <c:if test="${status.count*page.count-page.start<=20 && status.count*page.count-page.start>=-10}">
                                <li <c:if test="${status.index*page.count==page.start}">class="disabled"</c:if>>
                                    <a href="/credits?start=${status.index*page.count}${page.param}"
                        ...
                        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
                            <a href="/credits?start=${page.start+page.count}${page.param}" aria-label="Next">
                                <span aria-hidden="true">&rsaquo;</span>
                            </a>
                        </li>
                        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
                            <a href="/credits?start=${page.last}${page.param}" aria-label="Next">
