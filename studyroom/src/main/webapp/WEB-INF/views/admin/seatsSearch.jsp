                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
                            <a href="/seats/search?start=0${page.param}&&id=${condition.id}&&status=${conditon.status}" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>

                        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
                            <a href="/seats/search?start=${page.start-page.count}${page.param}&&id=${condition.id}&&status=${conditon.status}" aria-label="Previous">
                        ...
                        <c:forEach begin="0" end="${page.totalPage-1}" varStatus="status">
                            <c:if test="${status.count*page.count-page.start<=20 && status.count*page.count-page.start>=-10}">
                                <li <c:if test="${status.index*page.count==page.start}">class="disabled"</c:if>>
                                    <a href="/seats/search?start=${status.index*page.count}${page.param}&&id=${condition.id}&&status=${conditon.status}"
                        ...
                        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
                            <a href="/seats/search?start=${page.start+page.count}${page.param}&&id=${condition.id}&&status=${conditon.status}" aria-label="Next">
                                <span aria-hidden="true">&rsaquo;</span>
                            </a>
                        </li>
                        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
                            <a href="/seats/search?start=${page.last}${page.param}&&id=${condition.id}&&status=${conditon.status}" aria-label="Next">
