    ...
    <script>
        function searching(){
            ...
            if (title == "" && year == "" && month == "" && day == "" ){
                ...
                url = "/notices/search?title="+title+"&&date="+date;
    ...
    <div class="row" style="margin-top: 80px">
        <div class="col-sm-10 col-sm-offset-1">
            <div class="row" style="margin-top: 20px">
                ...
                " >查询信息</button>
                <button type ="button" id="create" onclick="window.location.href='/notices/create'" class="btn btn-success" style="margin-left:10px; font-size: 14px;font-weight: bold;
                " >新建通知</button>
            </div>
            <div class="row" style="margin-top: 20px">
                <table class="table table-striped table-bordered table-hover table-condensed">
                    ...
                    <c:forEach items="${notices}" var="n" >
                        <tr>
                            ...
                            <td><a href="/notices/detail?id=${n.id}">查看详情</a>&nbsp;&nbsp;<a href="/notices/update?id=${n.id}">修改</a>&nbsp;&nbsp;<a href="/notices/delete?id=${n.id}" onclick="return isDelete()">删除</a></td>
                        </tr>
                    </c:forEach>
                    </tbody>
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
                            <a href="/notices/search?start=0${page.param}&&title=${condition.title}&&date=${condition.date}" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>

                        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
                            <a href="/notices/search?start=${page.start-page.count}${page.param}&&title=${condition.title}&&date=${condition.date}" aria-label="Previous">
                        ...
                        <c:forEach begin="0" end="${page.totalPage-1}" varStatus="status">
                            <c:if test="${status.count*page.count-page.start<=20 && status.count*page.count-page.start>=-10}">
                                <li <c:if test="${status.index*page.count==page.start}">class="disabled"</c:if>>
                                    <a href="/notices/search?start=${status.index*page.count}${page.param}&&title=${condition.title}&&date=${condition.date}"
                        ...
                        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
                            <a href="/notices/search?start=${page.start+page.count}${page.param}&&title=${condition.title}&&date=${condition.date}" aria-label="Next">
                                <span aria-hidden="true">&rsaquo;</span>
                            </a>
                        </li>
                        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
                            <a href="/notices/search?start=${page.last}${page.param}&&title=${condition.title}&&date=${condition.date}" aria-label="Next">
