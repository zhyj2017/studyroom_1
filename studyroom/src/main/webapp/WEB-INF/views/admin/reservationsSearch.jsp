                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
                            <a href="/chosen/search?start=0${page.param}&&uid=${condition.uid}&&sid=${condition.sid}&&date=${condition.date}&&status=${condition.status}" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>

                        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
                            <a href="/chosen/search?start=${page.start-page.count}${page.param}&&uid=${condition.uid}&&sid=${condition.sid}&&date=${condition.date}&&status=${condition.status}" aria-label="Previous">
                        ...
                        <c:forEach begin="0" end="${page.totalPage-1}" varStatus="status">
                            <c:if test="${status.count*page.count-page.start<=20 && status.count*page.count-page.start>=-10}">
                                <li <c:if test="${status.index*page.count==page.start}">class="disabled"</c:if>>
                                    <a href="/chosen/search?start=${status.index*page.count}${page.param}&&uid=${condition.uid}&&sid=${condition.sid}&&date=${condition.date}&&status=${condition.status}"
                        ...
                        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
                            <a href="/chosen/search?start=${page.start+page.count}${page.param}&&uid=${condition.uid}&&sid=${condition.sid}&&date=${condition.date}&&status=${condition.status}" aria-label="Next">
                                <span aria-hidden="true">&rsaquo;</span>
                            </a>
                        </li>
                        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
                            <a href="/chosen/search?start=${page.last}${page.param}&&uid=${condition.uid}&&sid=${condition.sid}&&date=${condition.date}&&status=${condition.status}" aria-label="Next">
