...
<head>
    ...
    <script>
        $(document).ready(function() {
            ...
            for (i=0;i<seats.length;i++){
                var id = seats[i];
                var row = parseInt((id-1)/10)+1;
                var colomn = id - (row-1)*10;
                var x = row+'_'+colomn;
                seatlist.push(x);
            }
            ...
            //已预约的座位
            var rSeats = new Array();
            rSeats = ${reserveSeats};
            var rseatlist = new Array();
            rseatlist = [];
            for (i=0;i<rSeats.length;i++){
                var id = rSeats[i];
                var row = parseInt((id-1)/10)+1;
                var colomn = id - (row-1)*10;
                var x = row+'_'+colomn;
            ...
            sc.get(rseatlist).status('reserved');
    ...
    <script>
        function reserve() {
            var judge = ${isReserved};
            var warning = ${isWarning};
            var obj = document.getElementById('selected-seats').firstChild;
            if (!obj){
                alert("请先选择座位！");
            } else {
                console.log(judge);
                if (judge == true){
                    alert("你已在该时间段内预约，不可重复预约！");
                } else if(warning == true){
                    alert("你已被警告，不可预约，请联系管理员！");
                }
                else{

                    var sid = obj.getAttribute("name");
                    $.ajax({
                        url:"/reserve/add",
                        async:false,
                        datatype:"json",
...
<div class="container-fiuld">
    ...
    <div class="row" style="margin-top:5px">
        ...
        <div class="booking-details">
            <p>日期：<span>${reservation.date}</span></p>
            <p>时间：<span>${reservation.starttime}-${reservation.endtime}</span></p>
            <p>座位：</p>
            <ul id="selected-seats"></ul>
            <button type="button" class="btn btn-primary" style="font-size: 14px;font-weight: bold;margin-top:10px;margin-left: 60px" onclick="reserve()">预约</button>
            <div id="legend"></div>
        </div>
    </div>
</div>
